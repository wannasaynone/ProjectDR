...
namespace ProjectDR.Village.Storage
{
    ...
    /// 實作 IStorageQuery 供其他 Installer 透過 VillageContext.StorageReadOnly 查詢（ADR-003 D6）。
    /// </summary>
    public class StorageManager : IStorageQuery
    {
        /// <summary>
        ...
        // ===== IStorageQuery 介面實作（ADR-003 D6）=====

        /// <summary>
        /// IStorageQuery.HasItem — 判斷倉庫中是否有足夠數量的指定物品。
        /// 委派至 HasItem(itemId, requiredAmount)，供 ctx.StorageReadOnly 查詢使用。
        /// </summary>
        bool IStorageQuery.HasItem(string itemId, int requiredAmount)
        {
            return HasItem(itemId, requiredAmount);
        }
