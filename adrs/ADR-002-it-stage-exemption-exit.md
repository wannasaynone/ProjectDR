...
> **狀態**: Accepted (Executed)
> **提出日期**: 2026-04-21
> **最近更新**: 2026-04-22（Sprint 8 Wave 2.6b Full Exit PASS；狀態轉 Executed）
...
- Sprint 7（2026-04-22）= 獨立 Gate Sprint 的啟動 Sprint，完成 [A] 16 條 ConfigData 全處置但 [B][C] 發現結構不對齊（Sheets 21 分頁規範化 vs 現有 JSON 包裹物件），ADR-002 v1.6 標 Partial Exit
- Sprint 8（2026-04-22）= 獨立 Gate Sprint 的**最終執行 Sprint**，完成資料層徹底 IGameData 化（22 分頁 × 22 DTO）+ Sheets 對齊 + 匯出工具對齊；ADR-002 v1.7 升版 Accepted (Executed) = Full Exit PASS
- Sprint 7 + Sprint 8 合計為「獨立 Gate Sprint」的完整執行期
...
**每個改造項目的 Sub-checklist**（Sprint 7 + Sprint 8 已完成，A15/A17 豁免保留）：

- [x] C# class 加 `: IGameData`（15 活躍類別全做；A15/A17 豁免）
- [x] 加 `public int ID { get; }` 屬性
- [x] 語意字串主鍵類別加 `public string Key { get; }` 屬性
- [x] 更新 JSON 結構（增加 `id` 欄位 + Sprint 8 Wave 2 轉純陣列格式）
- [x] 更新 Sheets 對應分頁（增加 `id` 欄位；Wave 2.1 建 22 分頁 header）
- [x] 更新反序列化測試（加「實作 IGameData」斷言）
- [x] 透過 `GameStaticDataManager.Add<T>(handler)` 載入（構造器注入路徑；Wave 2.5 Installer 重構）
- [x] 查詢點改用 `GetGameData<T>(id)` 或構造器注入的不可變 Config 物件（Wave 2.5 完成）
- [x] VillageEntryPoint / 相關 Entry Point 組裝邏輯更新（Wave 2.5 `VillageEntryPointInstallers.cs` + `ExplorationEntryPoint.cs` 改用 JsonFx 靜態 API）
- [x] 檔頭移除 `// EXEMPT: ADR-002` 註解（僅 A15/A17 保留 EXEMPT 註解，屬原規劃豁免）
...
**B 區塊退出條件**（Sprint 8 Wave 2 完成）：
- [x] 15 個活躍 ConfigData（扣 A15/A17 豁免）+ 7 個 Sprint 8 Wave 2 拍板新增子表/metadata = **22 分頁**，全部欄位對齊 Sheets
- [x] Sheets 欄位命名與 C# property 轉換規則統一（Wave 2.5 改用 JsonFx 靜態 API `JsonReader.Deserialize<T[]>(json)`，Sheets snake_case 欄位 → JSON key snake_case → C# property snake_case 完全一致）
- [x] 每個分頁有資料（部分表如 `GiftSwords`/`Combat` 為 singleton 或設計即如此；不強制 ≥ 3，由 Sprint 8 spec § 3 拍板）
- [x] Sheets → JSON 匯出流程跑過至少一次成功（製作人 2026-04-22 於 Unity Editor `Start Convert` 產出 22 個 `.txt`，Play mode 啟動無 exception，全測試 1433/1433 PASS）

**此區塊 Sprint 8 收尾結果**：VS 設計師可透過 Sheets 熱改資料；Convert 流程通暢，資料層基礎設施完整。
...
- [x] **C04**：`/development-flow` Phase 1.5「資料源接入驗證」補完（`.claude/skills/development-flow/SKILL.md` Phase 1.5 區塊 6 項 checklist + 產出物格式）
- [x] **C05**：`DEV-DATA-INTAKE-REVIEW` gate 入 `director-gates.md`（行 229-265，含 8 項 DI checklist + 3 種 review mode 差異）
- [x] **C06**：PlayerPrefs 使用位點盤點（TR-save-001 已在 `tr-registry.yaml` 登記；notes 註明 ProjectDR CharacterIntroCGPlayer 使用為已知違規，待存檔系統設計後補 ADR；本 Gate 不強制清除）
- [x] **C07**：`tech-debt.md` 登記所有本 Gate 發現但未處理的技術債（13 筆，Sprint 8 Wave 2.6a 新增 TD-2026-012/013，TD-2026-011 已處置）

---

## [D] ADR / TR 綁定清單

- [x] **D01**：`tr-registry.yaml` 所有本批登記的 TR（TR-data-001/002/003、TR-save-001、TR-arch-001~008）都有 `governing_adrs` 綁定（TR-save-001 為空但 notes 註明待治理 ADR）
- [x] **D02**：ADR-001 狀態為 Accepted（v1.1 最近更新 2026-04-22，豁免條款收窄）
- [x] **D03**：`adrs/index.md` 建立並列出 ADR-001/002/003/004 共 4 條 Accepted
- [x] **D04**：`FILE_MAP.md` 同步本批新建檔案（tr-registry、4 條 ADR、data-governance-workflow-patches、Sprint 8 Wave 2 新增 22 DTO 路徑）
- [x] **D05**：`/create-control-manifest ProjectDR` 執行兩次（Sprint 7 初建 v1.0 + Sprint 8 Wave 2.6b 重建 v1.1），產出 `projects/ProjectDR/tech/control-manifest.md`
- [x] **D06**：本 ADR 狀態從 Accepted 轉為「已執行」— Status History v1.7 條目（2026-04-22 Sprint 8 Wave 2.6b）
...
| v1.7 | 2026-04-22 | Accepted (Executed) | **Sprint 8 Wave 2.6b Full Exit PASS**：[A] 17 條全處置（15 活躍 ConfigData 改 IGameData/拆子表 + 1 併入 + 1 刪除 + 2 原規劃豁免 A15/A17）；[B] Sheets **22 分頁**對齊（原預估 13 → Wave 2 spec 拍板擴充為 22 分頁 + 22 DTO，含 Q3/Q4/Q7 拆子表與 metadata 新分頁）；[C] 匯出流程跑過（製作人人工 Convert 產 22 個新 `.txt`；基礎設施 C01~C07 全 ✅，C02 spec v1.1 升版列 Sprint 8 C 區塊收尾工作）；[D] 治理完成（ADR-001/002/003/004 綁 12 條 TR；control-manifest v1.1 重建 50 條；本 ADR 狀態轉 Executed）。全測試 1433/1433 綠。A15（PlayerQuestionsConfigData）+ A17（HCGDialogueSetup）依原規劃保留為 VS 早期重寫對象。D4 TD-2026-011 VillageContext.gameDataAccess delegate 接線 Wave 2.6a 完成（真實 GameStaticDataManager 指向）；E2 `/architecture-review` PASS（TR 覆蓋率 91.7%、ADR 衝突 0、引擎相容性跨 ADR 一致）。ADR-002 退出 Gate 主持人（dev-head）確認通過，VS 階段啟動前置條件全部達標。|
