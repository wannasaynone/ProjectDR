...
//
// EXEMPT: ADR-002 A15
// 豁免理由：本 DTO 採語意字串主鍵（question_id），IT 階段後重寫時再改造為 IGameData + 雙欄位。
// 依 ADR-002 A15 明示豁免路線「IT 階段後重寫時豁免」。
// VS 階段重寫玩家問答系統時需改造（加 int id + string Key + GameStaticDataManager 載入）。
// 豁免日期：2026-04-22（E6 批次確認）
